...
void loop() {
	...
	if (BCURRENT != BLAST && BCURRENT == true) {
		...
		long itime = millis();
		while (!digitalRead(BUTTON)) {if (millis() - itime == 200) {oled.fillRect(0, 54, 128, 10, 1); oled.setCursor(1, 56); oled.setTextColor(0); oled.print("Back"); oled.setTextColor(1); oled.display();}}
		if (millis() - itime < 200) {
